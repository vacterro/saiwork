...
pub(crate) fn cleanup_all_remote_proxy_sessions_sync(app: &AppHandle) {
    let sessions = {
        if let Some(state) = app.try_state::<AppState>() {
            if let Ok(mut map) = state.remote_proxy_sessions.lock() {
                let sessions: Vec<String> = map.values().cloned().collect();
                map.clear();
                sessions
            } else {
                Vec::new()
    ...
    tauri::async_runtime::block_on(async move {
        for session_id in sessions {
            let _ = cleanup_remote_proxy_session(&app, &session_id).await;
        }
    });
}
...
fn main() {
    ...
    let log_dir = dirs::data_dir()
        .map(|p| p.join("ai.saipen.saiwork.client").join("logs"))
        .unwrap_or_else(|| std::path::PathBuf::from("logs"));
    std::fs::create_dir_all(&log_dir).ok();
    let file_appender = tracing_appender::rolling::daily(log_dir, "saiwork.log");
