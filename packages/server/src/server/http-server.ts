...
import {
  ...
  isWebSocketUpgradeClaimed,
  resolveUiDevWebSocketTarget,
} from "./websocket-upgrade-guard"
...
export function createHttpServer(deps: HttpServerDeps) {
  ...
  registerFreebuffGatewayRoutes(app, {
    ...
    liveModelIds: () => deps.freebuff.liveModelIds().then((ids) => ids ?? new Set<string>()),
  })
  ...
  setupWebSocketUpgradeGuard(app, {
    authManager: deps.authManager,
    logger: proxyLogger,
    uiDevServerUrl: deps.uiDevServerUrl,
  })
...
function setupSideCarWebSocketProxy(app: FastifyInstance, deps: SideCarWebSocketProxyDeps) {
  app.server.on("upgrade", (request, socket, head) => {
    ...
    claimWebSocketUpgrade(socket as Socket)
...
function setupWebSocketUpgradeGuard(
  app: FastifyInstance,
  deps: { authManager: AuthManager; logger: Logger; uiDevServerUrl?: string },
) {
  app.server.on("upgrade", (request, rawSocket, head) => {
    const socket = rawSocket as Socket
    guardWebSocketUpgrade(socket, (error) => {
      deps.logger.debug({ err: error, url: request.url }, "Websocket client socket errored")
    })

    if (isWebSocketUpgradeClaimed(socket) || !deps.uiDevServerUrl) return
    if (!isWebSocketUpgradeRequest(request)) {
      rejectUpgrade(socket, 400, "Bad Request")
      return
    }
    if (!deps.authManager.getSessionFromHeaders(request.headers)) {
      rejectUpgrade(socket, 401, "Unauthorized")
      return
    }

    const targetUrl = resolveUiDevWebSocketTarget(request.url, deps.uiDevServerUrl)
    if (!targetUrl) {
      rejectUpgrade(socket, 400, "Bad Request")
      return
    }
    ...
    proxyTargetWebSocketUpgrade({
      ...
      targetUrl,
      logger: deps.logger,
      logContext: { source: "ui-dev-server" },
      proxyLabel: "UI dev server",
      stripCookies: true,
    })
  })
}
