...
function runLinuxScript(spawnCommand: SpawnCommand, script: string, args: string[], timeoutMs: number,
  ...
  return distro
    ? spawnCommand("wsl.exe", ["--distribution", distro, "--exec", "sh", "-c", script, label, ...args], { encoding: "utf8", timeout: timeoutMs, windowsHide: true })
    : spawnCommand("sh", ["-c", script, label, ...args], { encoding: "utf8", timeout: timeoutMs, windowsHide: true })
}
...
export function probePosixProcesses(spawnCommand: SpawnCommand, timeoutMs: number,
  ...
  return querySnapshot(
    () => spawnCommand("ps", ["-axo", "pid=,ppid=,pgid=,lstart=,comm="], {
      encoding: "utf8", timeout: timeoutMs, env: { ...process.env, LC_ALL: "C", LANG: "C" }, windowsHide: true,
    }),
...
export function probeWindowsProcesses(spawnCommand: SpawnCommand, timeoutMs: number): ProcessSnapshot {
  ...
  return querySnapshot(
    () => spawnCommand("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script], { encoding: "utf8", timeout: timeoutMs, windowsHide: true }),
